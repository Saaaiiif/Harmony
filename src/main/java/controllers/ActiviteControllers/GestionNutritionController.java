...
public class GestionNutritionController {
    ...
    private VBox createAlimentCard(Aliment al) {
        ...
        card.setStyle("-fx-background-color: white; -fx-background-radius: 15px; -fx-padding: 20px; " +
        ...
        String macrosText = String.format("🥩 P: %.1f | 🍞 G: %.1f | 🥑 L: %.1f",
                al.getProteines(), al.getGlucides(), al.getLipides());
    ...
    void ajouterAliment(ActionEvent event) {
        ...
        try {
            ...
            VBox dialog = createPremiumDialog("✨", "Nouvel Aliment",
                    "Confirmez-vous l'ajout de : " + nom + " ?", "#10b981", "#d1fae5");

            Button btnConfirmer = createPremiumButton("Confirmer l'ajout", "linear-gradient(from 0% 0% to 100% 0%, #10b981, #059669)");
            btnConfirmer.setOnAction(e -> {
                service.ajouter(new Aliment(nom, calories, pro, glu, lip));
                nomField.clear(); caloriesField.clear(); proteinesField.clear();
                glucidesField.clear(); lipidesField.clear();
    ...
    private void ouvrirModalModification(Aliment al) {
        VBox dialog = createPremiumDialog("✏️", "Modification",
                "Mettez à jour les valeurs de l'aliment", "#3b82f6", "#dbeafe");

        TextField editNom = new TextField(al.getNom_aliment());          editNom.setPromptText("Nom");          styleDialogInput(editNom);
        TextField editCal = new TextField(String.valueOf(al.getCalories_pour_100g())); editCal.setPromptText("Calories");     styleDialogInput(editCal);
        TextField editPro = new TextField(String.valueOf(al.getProteines()));          editPro.setPromptText("Protéines (g)"); styleDialogInput(editPro);
        TextField editGlu = new TextField(String.valueOf(al.getGlucides()));           editGlu.setPromptText("Glucides (g)");  styleDialogInput(editGlu);
        TextField editLip = new TextField(String.valueOf(al.getLipides()));            editLip.setPromptText("Lipides (g)");   styleDialogInput(editLip);
        ...
        Button btnSauvegarder = createPremiumButton("Mettre à jour", "linear-gradient(from 0% 0% to 100% 0%, #3b82f6, #2563eb)");
        btnSauvegarder.setOnAction(e -> {
            try {
                ...
                // saisie invalide : on ne ferme pas
            }
        });
    ...
    private void demanderSuppression(Aliment al) {
        VBox dialog = createPremiumDialog("🗑️", "Zone de danger",
                "La suppression de '" + al.getNom_aliment() + "' est irréversible. Continuer ?",
                "#ef4444", "#fee2e2");

        Button btnSupprimer = createPremiumButton("Oui, supprimer", "linear-gradient(from 0% 0% to 100% 0%, #ef4444, #dc2626)");
    ...
    private VBox createPremiumDialog(String emoji, String titleStr, String descStr, String iconColor, String iconBg) {
        ...
        dialog.setStyle("-fx-background-color: linear-gradient(from 0% 0% to 100% 100%, #ffffff, #f9fafb); " +
                "-fx-background-radius: 25px; -fx-padding: 40px; " +
                "-fx-effect: dropshadow(three-pass-box, rgba(0,0,0,0.2), 30, 0, 10, 15);");
        dialog.setMaxWidth(450);
        dialog.setAlignment(Pos.CENTER);

        Label icon = new Label(emoji);
        icon.setStyle("-fx-font-size: 35px; -fx-background-color: " + iconBg + "; " +
                "-fx-text-fill: " + iconColor + "; -fx-padding: 15; -fx-background-radius: 50%;");
    ...
    private Button createPremiumButton(String text, String bgStyle) {
        ...
        btn.setStyle("-fx-background-color: " + bgStyle + "; -fx-text-fill: white; -fx-font-weight: bold; " +
                "-fx-font-size: 14px; -fx-padding: 12 25; -fx-background-radius: 30px; -fx-cursor: hand; " +
