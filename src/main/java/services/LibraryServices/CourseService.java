...
public class CourseService {
    ...
    public List<SubjectRow> listSubjects() throws Exception {
        List<SubjectRow> out = new ArrayList<>();
        try (Connection conn = DB.getConnection();
             PreparedStatement ps = conn.prepareStatement("SELECT id, name FROM subject ORDER BY name");
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) out.add(new SubjectRow(rs.getInt("id"), rs.getString("name")));
        }
        return out;
    }
